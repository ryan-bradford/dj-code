...
var BLACK = 0x000000;
var ORANGE = 0xFFA500;
var LIGHT_GREEN = 0x90EE90;
var WHITE = 0xFFFFFF;
var DARK_GREEN = 0x023020;
var PURPLE = 0xA020F0;
var PINK = 0xFFC0CB;
var LIGHT_BLUE = 0xADD8E6;
var DARK_BLUE = 0x00008B;
var BROWN = 0x964B00;
var RED = 0xff0000;

// Buttons are rows of 10 starting at 11
var LaunchpadMkii = /** @class */ (function () {
    function LaunchpadMkii() {
        var _a;
    ...
    LaunchpadMkii.prototype.init = function () {
        this.buttonWatchers = new Map();
        this.subscriptionToMidiId = new Map();
        for (var x = 0; x < 9; x++) {
            for (var y = 0; y < 9; y++) {
                this.changeButtonColor(x, y, BLACK);
                this.buttonWatchers.set(this.getButtonMidiId(x, y), new Map());
            }
        }
    };
    LaunchpadMkii.prototype.shutdown = function () {
        this.buttonWatchers = new Map();
        this.subscriptionToMidiId = new Map();
        for (var x = 0; x < 9; x++) {
            for (var y = 0; y < 9; y++) {
    ...
    LaunchpadMkii.prototype.buttonPressed = function (midiNumber, event) {
        this.buttonWatchers.get(midiNumber).forEach(function (callback) {
            if (callback) {
                callback(event);
            }
        });
    };
    LaunchpadMkii.prototype.watchButtonPressed = function (x, y, callback) {
    ...
    LaunchpadMkii.prototype.stopWatchingButton = function (subscriptionId) {
        this.buttonWatchers.get(this.subscriptionToMidiId.get(subscriptionId))
            .set(subscriptionId, undefined);
    };
    LaunchpadMkii.prototype.changeButtonColor = function (x, y, color) {
        var midiNumber = this.getButtonMidiId(x, y);
        if (midiNumber >= 104) {
     ...
     * 0 indexed
     */
    LaunchpadMkii.prototype.getButtonMidiId = function (x, y) {
        if (y === 8) {
            return 104 + x;
        }
        else {
    ...
    LaunchpadMkii.prototype.getButtonX = function (midiNumber) {
        if (midiNumber >= 104) {
...
var LaunchpadMapping = /** @class */ (function () {
    function LaunchpadMapping(launchpad, mixxx) {
        this.launchpad = launchpad;
        this.mixxx = mixxx;
        this.leftMixxxSubscriptions = new Array();
        this.leftButtonSubscriptions = new Array();
        this.rightMixxxSubscriptions = new Array();
        this.rightButtonSubscriptions = new Array();
        this.globalButtonSubscription = new Array();
    }
    LaunchpadMapping.prototype.initMapping = function () {
        var _this = this;
        this.configureDeckMapping(1);
        this.configureDeckMapping(2);
        this.globalButtonSubscription.forEach(function (sub) { return _this.launchpad.stopWatchingButton(sub); });
        this.globalButtonSubscription = new Array();
        this.configureForBeatloopToggleButton();
        this.configureForMasterVolume();
        this.configureForHeadphoneVolume();
    };
    LaunchpadMapping.prototype.configureDeckMapping = function (deckNumber) {
        var _this = this;
        var xOffset = 0;
        if (deckNumber === 2 || deckNumber === 4) {
            xOffset += 4;
            this.rightMixxxSubscriptions.forEach(function (subscription) { return subscription.disconnect(); });
            this.rightButtonSubscriptions.forEach(function (subscription) { return _this.launchpad.stopWatchingButton(subscription); });
        }
        if (deckNumber === 1 || deckNumber === 3) {
            this.leftMixxxSubscriptions.forEach(function (subscription) { return subscription.disconnect(); });
            this.leftButtonSubscriptions.forEach(function (subscription) { return _this.launchpad.stopWatchingButton(subscription); });
        }
        ...
        engine.beginTimer(100, function () {
            if (deckNumber === 1) {
                _this.configureForDeckSwapButton(3, deckNumber);
            }
            else if (deckNumber === 2) {
                _this.configureForDeckSwapButton(4, deckNumber);
            }
            else if (deckNumber === 3) {
                _this.configureForDeckSwapButton(1, deckNumber);
            }
            else if (deckNumber === 4) {
                _this.configureForDeckSwapButton(2, deckNumber);
            }
        }, 1);
    };
    LaunchpadMapping.prototype.configurePlayButton = function (xOffset, deck) {
        var _this = this;
        var launchpadSubscription = this.launchpad.watchButtonPressed(xOffset, 0, function (event) {
            if (event === ButtonStatus.PRESSED) {
                _this.mixxx.togglePlay(deck);
            }
        });
        var mixxSubscription = this.mixxx.subscribeToPlayStatus(deck, function (status) {
            if (status === PlayStatus.PAUSED) {
                _this.launchpad.changeButtonColor(xOffset, 0, RED);
            }
            else {
                _this.launchpad.changeButtonColor(xOffset, 0, LIGHT_GREEN);
            }
        });
        this.trackSubscriptions(launchpadSubscription, mixxSubscription, deck);
    };
    LaunchpadMapping.prototype.configureCueButton = function (xOffset, deck) {
        var _this = this;
        var launchpadSubscription = this.launchpad.watchButtonPressed(xOffset + 1, 0, function (event) {
            _this.mixxx.touchCue(deck, event == ButtonStatus.PRESSED);
        });
        var mixxSubscription = this.mixxx.subscribeToCueStatus(deck, function (status) {
            if (status === CueStatus.UNSET) {
                _this.launchpad.changeButtonColor(xOffset + 1, 0, RED);
            }
            else {
                _this.launchpad.changeButtonColor(xOffset + 1, 0, LIGHT_GREEN);
            }
        });
        this.trackSubscriptions(launchpadSubscription, mixxSubscription, deck);
    };
    LaunchpadMapping.prototype.configureForAllBeatloopButtons = function (xOffset, deck) {
        var baseLoop = 0.125;
    ...
    LaunchpadMapping.prototype.configureForBeatloopButton = function (x, y, size, deck) {
        var _this = this;
        var launchpadSubscription = this.launchpad.watchButtonPressed(x, y, function (event) {
            _this.mixxx.toggleBeatloop(deck, size, event === ButtonStatus.PRESSED);
        });
        var mixxSubscription = this.mixxx.subscribeToBeatloopStatus(deck, size, function (status) {
            if (status === LoopStatus.ENABLED) {
                _this.launchpad.changeButtonColor(x, y, LIGHT_BLUE);
            }
            else {
                _this.launchpad.changeButtonColor(x, y, DARK_BLUE);
            }
        });
        this.trackSubscriptions(launchpadSubscription, mixxSubscription, deck);
    };
    LaunchpadMapping.prototype.configureForBeatloopToggleButton = function () {
        var _this = this;
        this.launchpad.changeButtonColor(8, 0, WHITE);
        var launchpadSubscription = this.launchpad.watchButtonPressed(8, 0, function (event) {
            if (event === ButtonStatus.PRESSED) {
                _this.mixxx.toggleLoopType();
            }
            if (_this.mixxx.getLoopType() === LoopType.SLIP) {
                _this.launchpad.changeButtonColor(8, 0, PINK);
            }
            else {
                _this.launchpad.changeButtonColor(8, 0, WHITE);
            }
        });
        this.globalButtonSubscription.push(launchpadSubscription);
    };
    LaunchpadMapping.prototype.configureForAllHotcueButtons = function (xOffset, deck) {
        for (var i = 1; i <= 4; i++) {
    ...
    LaunchpadMapping.prototype.configureForHotcueButton = function (x, y, hotcueId, deck) {
        var _this = this;
        var launchpadSubscription = this.launchpad.watchButtonPressed(x, y, function (event) {
            _this.mixxx.toggleHotcue(deck, hotcueId, event === ButtonStatus.PRESSED);
        });
        var mixxSubscription = this.mixxx.subscribeToHotcueColor(deck, hotcueId, function (color) {
            if (_this.mixxx.getHotcueEnabled(deck, hotcueId)) {
                _this.launchpad.changeButtonColor(x, y, color);
            }
            else {
                _this.launchpad.changeButtonColor(x, y, BLACK);
            }
        });
        var mixxSubscription2 = this.mixxx.subscribeToHotcueEnabled(deck, hotcueId, function (enabled) {
            if (enabled === 0) {
                _this.launchpad.changeButtonColor(x, y, BLACK);
            }
            else {
                _this.launchpad.changeButtonColor(x, y, _this.mixxx.getHotcueColor(deck, hotcueId));
            }
        });
        this.trackSubscriptions(launchpadSubscription, mixxSubscription, deck);
        this.trackSubscriptions(undefined, mixxSubscription2, deck);
    };
    LaunchpadMapping.prototype.configureForDeckSwapButton = function (toSwitchTo, currentDeck) {
        var _this = this;
        if (toSwitchTo === 1 || toSwitchTo === 3) {
            this.updateDeckToggledDisplay(currentDeck, 0);
            var launchpadSubscription = this.launchpad.watchButtonPressed(0, 8, function (event) {
                if (event != ButtonStatus.PRESSED) {
                    return;
                }
                _this.configureDeckMapping(toSwitchTo);
                _this.updateDeckToggledDisplay(toSwitchTo, 0);
            });
            this.leftButtonSubscriptions.push(launchpadSubscription);
        }
        else {
            this.updateDeckToggledDisplay(currentDeck, 1);
            var launchpadSubscription = this.launchpad.watchButtonPressed(1, 8, function (event) {
                if (event != ButtonStatus.PRESSED) {
                    return;
                }
                _this.configureDeckMapping(toSwitchTo);
                _this.updateDeckToggledDisplay(toSwitchTo, 1);
            });
            this.rightButtonSubscriptions.push(launchpadSubscription);
        }
    };
    LaunchpadMapping.prototype.configureForMasterVolume = function () {
        var _this = this;
        this.launchpad.changeButtonColor(8, 7, RED);
        this.launchpad.changeButtonColor(8, 6, LIGHT_GREEN);
        var timer = undefined;
        var volumeUpSubscription = this.launchpad.watchButtonPressed(8, 7, function (event) {
            if (event != ButtonStatus.PRESSED) {
                engine.stopTimer(timer);
                return;
            }
            timer = engine.beginTimer(100, function () {
                _this.mixxx.changeMasterVolume("up");
            });
        });
        var volumeDownSubscription = this.launchpad.watchButtonPressed(8, 6, function (event) {
            if (event != ButtonStatus.PRESSED) {
                engine.stopTimer(timer);
                return;
            }
            timer = engine.beginTimer(100, function () {
                _this.mixxx.changeMasterVolume("down");
            });
        });
        this.globalButtonSubscription.push(volumeUpSubscription);
        this.globalButtonSubscription.push(volumeDownSubscription);
    };
    LaunchpadMapping.prototype.configureForHeadphoneVolume = function () {
        var _this = this;
        this.launchpad.changeButtonColor(8, 5, RED);
        this.launchpad.changeButtonColor(8, 4, LIGHT_GREEN);
        var timer = undefined;
        var volumeUpSubscription = this.launchpad.watchButtonPressed(8, 5, function (event) {
            if (event != ButtonStatus.PRESSED) {
                engine.stopTimer(timer);
                return;
            }
            timer = engine.beginTimer(100, function () {
                _this.mixxx.changeHeadphoneVolume("up");
            });
        });
        var volumeDownSubscription = this.launchpad.watchButtonPressed(8, 4, function (event) {
            if (event != ButtonStatus.PRESSED) {
                engine.stopTimer(timer);
                return;
            }
            timer = engine.beginTimer(100, function () {
                _this.mixxx.changeHeadphoneVolume("down");
            });
        });
        this.globalButtonSubscription.push(volumeUpSubscription);
        this.globalButtonSubscription.push(volumeDownSubscription);
    };
    LaunchpadMapping.prototype.updateDeckToggledDisplay = function (newDeck, xOffset) {
        if (newDeck === 3 || newDeck === 4) {
            this.launchpad.changeButtonColor(xOffset, 8, LIGHT_BLUE);
        }
        else {
            this.launchpad.changeButtonColor(xOffset, 8, PURPLE);
        }
    };
    LaunchpadMapping.prototype.trackSubscriptions = function (launchpadSubscription, mixxxSubscription, deck) {
        if (deck === 1 || deck === 3) {
    ...
    return LaunchpadMapping;
}());

// eslint-disable-next-line no-var
var mixxx = new Mixxx();
var launchpad = new LaunchpadMkii();
var mapping = new LaunchpadMapping(launchpad, mixxx);
var MyController = {
    ...
    buttonPressed: function (channel, control, value, status, group) {
        launchpad.buttonPressed(Number.parseInt(control), value);
    }
};
if (module$1) {
    module$1.exports = MyController;
}
var module$1 = {};
